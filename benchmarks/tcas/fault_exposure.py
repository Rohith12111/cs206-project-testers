...
def compile_c_file(folder_path, c_file_name):
    compile_command = f"gcc -Wno-return-type -g -o tcas {folder_path}/tcas.c"
...
def run_test_suite(test_suite_file):
    ...
    for command in test_suite_commands:
        result = subprocess.run(f"./tcas {command.strip()}", shell=True, stdout=subprocess.PIPE,encoding='ISO-8859-1', text=True)
...
def compile_base_program():
    compile_command = "gcc -Wno-return-type -g -o tcas tcas.c"
    subprocess.run(compile_command, shell=True, check=True)
...
def main():
    ...
    if args.tsc:
        ...
        base_path = '../benchmarks/tcas/'  
        base_run(test_suite_file)
        results = process_folders(test_suite_file)
        with open('json/fault_exposure_tsc_tcas.json', 'w') as file:
        ...
        base_run(test_suite_file)
        results = process_folders(test_suite_file)
        with open('json/fault_exposure_tbc_tcas.json', 'w') as file:
            json.dump(results, file, indent=4)
        ...
        base_run(test_suite_file)
        results = process_folders(test_suite_file)
        with open('json/fault_exposure_asc_tcas.json', 'w') as file:
            json.dump(results, file, indent=4)
        ...
        base_run(test_suite_file)
        results = process_folders(test_suite_file)
        with open('json/fault_exposure_abc_tcas.json', 'w') as file:
            json.dump(results, file, indent=4)
        ...
        base_run(test_suite_file)
        results = process_folders(test_suite_file)
        with open('json/fault_exposure_rsc_tcas.json', 'w') as file:
            json.dump(results, file, indent=4)
        ...
        base_run(test_suite_file)
        results = process_folders(test_suite_file)
        with open('json/fault_exposure_rbc_tcas.json', 'w') as file:
            json.dump(results, file, indent=4)
        ...
        base_run(test_suite_file)
        results = process_folders(test_suite_file)
        with open('json/fault_exposure_universe_tcas.json', 'w') as file:
            json.dump(results, file, indent=4)
