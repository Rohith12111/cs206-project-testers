...
def base_coverage():
    ...
    with open('json/additional_replace.json', 'r') as file:
    ...
    with open('json/base_coverage_additional_replace.json', 'w') as outfile:
...
def test_suite_generation(coverage):
    ...
    with open('json/base_coverage_additional_replace.json', 'r') as file:
        base_coverage_list = json.load(file)
    base_coverage_set = set(base_coverage_list)
    temp_coverage_set = set()
    testsuit = []

    while temp_coverage_set != base_coverage_set and sorted_replace_map:
        max_diff = 0
        max_test = None
        for key, value in sorted_replace_map.items():
            ...
            if len(difference) > max_diff:
                ...
                max_test = key
        if max_test:
            temp_coverage_set.update(set(sorted_replace_map[max_test]))
            testsuit.append(max_test)
            del sorted_replace_map[max_test]
...
def main():
    ...
    for input_file in input_files:
        ...
        if args.s:
            ...
            delete_file()
            with open('json/additional_replace.json', 'w') as file:
