...
def test_suite_generation(coverage):
    with open("json/random_printtokens.json",'r') as file:
        printtokens_random_map=json.load(file)
    
    with open('json/base_coverage_random_printtokens.json','r') as file:
        base_coverage_list=json.load(file)
    base_coverage_set=set(base_coverage_list)
    temp_coverage_set=set()
    testsuit=[]
    for key,value in printtokens_random_map.items():
...
def base_coverage():
    ...
    with open('json/base_coverage_random_printtokens.json', 'w') as outfile:
...
def main():
    ...
    for input_file in input_files:
        ...
        if args.s:
            ...
            delete_file()
            with open('json/random_printtokens.json', 'w') as file:
