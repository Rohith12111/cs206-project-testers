...
def base_coverage():
    ...
    with open('json/additional_printtokens2.json', 'r') as file:
    ...
    with open('json/base_coverage_additional_printtokens2.json', 'w') as outfile:
...
def test_suite_generation(coverage):
    ...
    with open("json/additional_printtokens2.json", 'r') as file:
        data = json.load(file)
    sorted_printtokens2_map = dict(sorted(data.items(), key=lambda value: len(value[1]), reverse=True))

    with open('json/base_coverage_additional_printtokens2.json', 'r') as file:
        base_coverage_list = json.load(file)
    base_coverage_set = set(base_coverage_list)
    temp_coverage_set = set()
    testsuit = []

    while temp_coverage_set != base_coverage_set and sorted_printtokens2_map:
        max_diff = 0
        max_test = None
        for key, value in sorted_printtokens2_map.items():
            ...
            if len(difference) > max_diff:
                ...
                max_test = key
        if max_test:
            temp_coverage_set.update(set(sorted_printtokens2_map[max_test]))
            testsuit.append(max_test)
            del sorted_printtokens2_map[max_test]
...
def main():
    ...
    for input_file in input_files:
        ...
        if args.s:
            ...
            delete_file()
            with open('json/additional_printtokens2.json', 'w') as file:
