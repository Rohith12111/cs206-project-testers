...
def compile_c_file(folder_path, c_file_name):
    compile_command = f"gcc -Wno-return-type -g -o totinfo {folder_path}/totinfo.c -lm"
...
def run_test_suite(test_suite_file):
    ...
    for command in test_suite_commands:
        result = subprocess.run(f"./totinfo {command.strip()}", shell=True, stdout=subprocess.PIPE,encoding='ISO-8859-1', text=True)
...
def compile_base_program():
    compile_command = "gcc -Wno-return-type -g -o totinfo totinfo.c -lm"
    subprocess.run(compile_command, shell=True, check=True)
...
def main():
    ...
    if args.tsc:
        ...
        base_path = '../benchmarks/totinfo/'  
        base_run(test_suite_file)
        results = process_folders(test_suite_file)
        with open('json/fault_exposure_tsc_totinfo.json', 'w') as file:
        ...
        base_run(test_suite_file)
        results = process_folders(test_suite_file)
        with open('json/fault_exposure_tbc_totinfo.json', 'w') as file:
            json.dump(results, file, indent=4)
        ...
        base_run(test_suite_file)
        results = process_folders(test_suite_file)
        with open('json/fault_exposure_asc_totinfo.json', 'w') as file:
            json.dump(results, file, indent=4)
        ...
        base_run(test_suite_file)
        results = process_folders(test_suite_file)
        with open('json/fault_exposure_abc_totinfo.json', 'w') as file:
            json.dump(results, file, indent=4)
        ...
        base_run(test_suite_file)
        results = process_folders(test_suite_file)
        with open('json/fault_exposure_rsc_totinfo.json', 'w') as file:
            json.dump(results, file, indent=4)
        ...
        base_run(test_suite_file)
        results = process_folders(test_suite_file)
        with open('json/fault_exposure_rbc_totinfo.json', 'w') as file:
            json.dump(results, file, indent=4)
        ...
        base_run(test_suite_file)
        results = process_folders(test_suite_file)
        with open('json/fault_exposure_universe_totinfo.json', 'w') as file:
            json.dump(results, file, indent=4)
