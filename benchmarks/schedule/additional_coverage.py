...
def compile_program():
    compile_command = "gcc -fprofile-arcs -ftest-coverage -Wno-return-type -g -o schedule schedule.c"
    subprocess.run(compile_command, shell=True, check=True)

def generate_statement_gcov(input_file):
    run_command = f"./schedule {input_file}"
    try:
        subprocess.run(run_command, shell=True, check=True, stderr=subprocess.PIPE)
    except subprocess.CalledProcessError as e:
        pass
    gcov_command = "gcov -r schedule.c"
...
def generate_branch_gcov(input_file):
    ...
    try:
        ...
        pass
    gcov_command = "gcov -b schedule.c"
    subprocess.run(gcov_command, shell=True, check=True)
...
def delete_file():
    subprocess.run("rm -rf schedule.c.gcov schedule.gcov.json schedule schedule.gcda schedule.gcno", shell=True, check=True)
    subprocess.run("ls")

def base_coverage():
    covered_statements_set = set()
    with open('json/additional_schedule.json', 'r') as file:
    ...
    with open('json/base_coverage_additional_schedule.json', 'w') as outfile:
...
def test_suite_generation(coverage):
    ...
    with open("json/additional_schedule.json", 'r') as file:
        data = json.load(file)
    sorted_schedule_map = dict(sorted(data.items(), key=lambda value: len(value[1]), reverse=True))

    with open('json/base_coverage_additional_schedule.json', 'r') as file:
        base_coverage_list = json.load(file)
    base_coverage_set = set(base_coverage_list)
    temp_coverage_set = set()
    testsuit = []

    while temp_coverage_set != base_coverage_set and sorted_schedule_map:
        max_diff = 0
        max_test = None
        for key, value in sorted_schedule_map.items():
            ...
            if len(difference) > max_diff:
                ...
                max_test = key
        if max_test:
            temp_coverage_set.update(set(sorted_schedule_map[max_test]))
            testsuit.append(max_test)
            del sorted_schedule_map[max_test]
...
def main():
    ...
    for input_file in input_files:
        ...
        if args.s:
            ...
            coverable_lines = parse_statement_gcov_file("schedule.c.gcov")
            coverable_lines_map[input_file] = coverable_lines
            delete_file()
            with open('json/additional_schedule.json', 'w') as file:
            ...
            coverable_lines = parse_branch_gcov_file("schedule.c.gcov")
            coverable_lines_map[input_file] = coverable_lines
            delete_file()
            with open('json/additional_schedule.json', 'w') as file:
                json.dump(coverable_lines_map, file, indent=4)
